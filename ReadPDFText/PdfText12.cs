...
{
	public class PdfText12 : IPdfTextXX
	{
		public const bool SHOW_DEBUG_INFO = false;
		public const bool SHOW_DEBUG_SHEET_DATA = false;
		public const bool SHOW_ORIGIN_MARKER = false;


		public const string PAGES_PG_NUM_FORMAT = "*{0:D5}";

		// misc fields
		public float pi180 = (float) Math.PI;
		public float pi90;
		public float pi270;

		public int rectangleIdx { get; set; }= 0;
		public bool? doPageRotate { get; set; }= false;
		...
		public int Mergedone => PdfText12.MERGEDONE;
		public int Validatedone => PdfText12.VALIDATEDONE;
		public int Bookmarksdone => PdfText12.BOOKMARKSDONE;

		public string[] Elt => PdfText12.elt;

		public  List<PdfTreeLeaf> RotFailList => PdfText12.RotationFailList;
		...
		private bool validate()
		{
			...
			v12 = new Validate12(destPdfDoc, 
				shtNumMinLen, shtNumMaxLen, 
				normalPgOrientationIsVert,
				this);
			...
			foreach (KeyValuePair<string, PdfTreeLeaf> kvp in pages)
			{
				...
				result = v12.validate(kvp.Value) && result;
			}
		...
		{
			// Debug.WriteLine("make bookmarks");
...
// 		// }
	...
	#region step 4 - find sheet references
		...
		{
			// Debug.WriteLine("locate sheet refs");
			...
			ListTextExtractionFilter3 filter3 = new ListTextExtractionFilter3(pages, v12.shtNumMinLen, v12.shtNumMaxLen);
			...
			for (int i = 1; i <= destPdfDoc.GetNumberOfPages(); i++)
			{
				...
				Debug.WriteLine($"\n*** getting xrefs| {leaf.SheetNumber}");
				...
				strat3.OnPage = i;
				strat3.SheetNumberTals = leaf.SheetNumberTals;
				...
				string a = PdfTextExtractor.GetTextFromPage(page, strat3);

				// save the list of sheet references found
				leaf.PageLinkData = strat3.list;

				// Debug.WriteLine($"\n\n*** space character counts| preface {strat3.PrefaceSpaceCounts} | suffix {strat3.SuffixSpaceCounts}\n\n");
	...
	#region step 5 - add annot and links
		...
		private bool addAnnoAndLinks(PdfTreeLeaf leaf, PdfLayer layer)
		{
			...
			// pd = page.GetPdfObject();
			//
			// PdfNumber pn = pd.GetAsNumber(PdfName.UserUnit);
			//
			// if (pn != null)
			// {
			// 	Debug.WriteLine($"user unit| {pn.GetValue().ToString()}");
			// }
			// else
			// {
			// 	Debug.WriteLine("no user unit");
			// }

			Debug.WriteLine($"page rotation| {pageRotation}");
			Rectangle r = page.GetPageSize();
			Debug.WriteLine($"page size no rot| w {r.GetWidth()} | h {r.GetHeight()}");
			r = page.GetPageSizeWithRotation();
			Debug.WriteLine($"page size w/ rot| w {r.GetWidth()} | h {r.GetHeight()}");
			r = page.GetCropBox();
			Debug.WriteLine($"crop size| w {r.GetWidth()} | h {r.GetHeight()}");
			...
			//
			// CanvasGraphicsState g = pdfCanvas.GetGraphicsState();
			//
			// Matrix m = g.GetCtm();
			//
			// Debug.WriteLine($"CTM | {m.Get(0)} | {m.Get(1)} | {m.Get(2)} | {m.Get(3)} | {m.Get(4)} | {m.Get(5)} | {m.Get(6)} | {m.Get(7)} | {m.Get(8)}");
		...
		// adds annotations but no link / link rectangles
		...
		private void placeFooter(PdfTreeLeaf leaf)
		{
			...
			Rectangle r = leaf.SheetData.FooterRect[rectangleIdx];

			Debug.WriteLine($"footer rect (rotated)| {ReadPDFText.fmtRect(r)}");
			...
			// }
			//
			// Debug.WriteLine($"footer anno location| x {x} | y {y} | pg sz| w {pageSize.GetWidth()} | h {pageSize.GetHeight()} |   rotation {rotation} pg rotation {pageRotation}");
...
// rectangle
			...
			// float rotation = 0;
			//
			// float x = leaf.SheetData.DisclaimerRect[rectangleIdx].GetX() ;
			// float y = leaf.SheetData.DisclaimerRect[rectangleIdx].GetY() + 4f;


			// Rectangle r = rotateIfNeeded(leaf.SheetData.DisclaimerRect[rectangleIdx]);
			Rectangle r = leaf.SheetData.DisclaimerRect[rectangleIdx];

			Debug.WriteLine($"disclaimer rect (rotated)| {ReadPDFText.fmtRect(r)}");
			...
			// 	y += r.GetHeight() / 2;
			//
			// 	Debug.WriteLine($"disclaimer at {pageRotation}  ({rotation})");
			// } 
			// else if (pageRotation == 0)
			// {
			// 	angle = 90f;
			// 	rotation = FloatOps.ToRad(90f);
			//
			// 	x = r.GetX() + r.GetWidth() / 2;
			// 	y = r.GetY()+ r.GetHeight();
			//
			...
			// {
			// 	angle = 180f;
			// 	rotation = FloatOps.ToRad(180);
			// 	
			// 	y += r.GetHeight() / 2;
			//
			// 	Debug.WriteLine($"disclaimer at {pageRotation}  ({rotation})");
			// }
			// Debug.WriteLine($"disclaimer anno location x {x} | y {y} | rot {rotation}  ({angle})");
		...
		private void showLinkLocation(Rectangle r)
		{
			Debug.WriteLine($"location| x {r.GetX()} | y {r.GetY()} | w {r.GetWidth()} | h {r.GetHeight()}");
		}
...
// rectangle
			...
			PdfAnnotation anno = makeUrlLinkAnnotation(r, cfg.DisclaimerUrl);


			Debug.WriteLine($"disclaimer anno & link| idx is rectangleIdx| {rectangleIdx}");
			// Rectangle rx = leaf.SheetData.DisclaimerRect[rectangleIdx];
			showLinkLocation(r);

			// Debug.WriteLine("Disclaimer link| idx = 0");
			//  r = leaf.SheetData.DisclaimerRect[rectangleIdx];
			// showLinkLocation(r);

// rectangle

			// original
			// placeUrlLinkOnLayer(page, canvas, layer, leaf.SheetData.DisclaimerRect[rectangleIdx], anno);


			placeUrlLinkOnLayer(page, canvas, layer, r, anno);
...
// rectangle
			// Rectangle r = rotateIfNeeded(leaf.SheetData.PrimaryAuthorRect[rectangleIdx]);
			Rectangle r = leaf.SheetData.PrimaryAuthorRect[rectangleIdx];


			PdfAnnotation anno = makeUrlLinkAnnotation(r, cfg.AuthorLinkUrl);
			...
			Debug.WriteLine($"author anno and link| idx is rectangleIdx| {rectangleIdx}");
			// Rectangle rx = leaf.SheetData.PrimaryAuthorRect[rectangleIdx];
			showLinkLocation(r);


			// Debug.WriteLine("author link| idx = 0");
			//  r = leaf.SheetData.PrimaryAuthorRect[0];
			// showLinkLocation(r);
...
// rectangle
			...
			// original
			// placeUrlLinkOnLayer(page, canvas, layer, leaf.SheetData.PrimaryAuthorRect[rectangleIdx], anno);
			// placeUrlLinkOnLayer(page, canvas, layer, leaf.SheetData.PrimaryAuthorRect[0], anno);

			Debug.WriteLine("author anno| idx = 0");
		}
...
// rectangle
			// Rectangle r = rotateIfNeeded(leaf.SheetData.SheetNumberLinkRect[rectangleIdx]);
			Rectangle r = leaf.SheetData.SheetNumberLinkRect[rectangleIdx];


			PdfAnnotation pa = makeLinkAnnotation(r, leaf.SheetNumberTals);
			...
			Debug.WriteLine($"return anno and link| idx is rectangleIdx| {rectangleIdx}");
			// Rectangle r = leaf.SheetData.SheetNumberLinkRect[rectangleIdx];
			showLinkLocation(r);
...
// ***
		...
		// need to fix?? - no provide a rotated rectangle
		private void placeRectangle(PdfCanvas canvas, Rectangle rr, Color strokeColor, Color fillColor, float fillOpacity = 1)
		{
...
/*
		...
		{
			if (pageRotation == 270) return r;

			Rectangle ps = pageSize;

			// Rectangle ps = pageSizeWithRotation;

			Debug.WriteLine($"before rotation| pg rot {pageRotation} | {ReadPDFText.fmtRect(r,1)}");
			...
			if (pageRotation == 0)
			{
				...
				y = r.GetX();
				x = ps.GetWidth() - r.GetY() - w;
			}
			else
			{
				// Debug.WriteLine($"rotate if needed| rotation {pageRotation} | not being rotated");

				w = r.GetWidth();
				h = r.GetHeight();

				// y = ps.GetWidth() - r.GetY() - h;
				// x = ps.GetHeight() - r.GetX() - w;

				x = ps.GetWidth() - r.GetX() - w;
				y = ps.GetHeight() - r.GetY() - h;


			}

			Rectangle rAdj = new Rectangle(x, y, w, h);

			Debug.WriteLine($"after rotation| pg rot {pageRotation} | {ReadPDFText.fmtRect(rAdj,1)}");

			return rAdj;
		}

		private Rectangle rotateIfNeeded2(Rectangle r)
		{
			if (pageRotation == 270) return r;
			...
			Rectangle rx = new Rectangle(r.GetY(), r.GetX(), r.GetHeight(), r.GetWidth());
...
*/
		...
		// private void rotateCoordinatIfNeeded(float x, float y, float w, float h, out float x1, out float y1)
		// {
		// 	if (doPageRotate == false)
		// 	{
		// 		x1 = x;
	...
	#region step 6 - add document info - meta data
		...
		// public void checkIfRotate(PdfPage page)
		// {
		...
		// 	rectangleIdx = 0;
		//
		// 	Debug.WriteLine($"do page rotation| {doPageRotate}  |  rect idx {rectangleIdx}");
		//
		// 	return;
		//
		// 	if (result == null)
		...
		//
		// 	Debug.WriteLine($"rotation| {pageRotation} | page size w/rot {ps.GetWidth()} x {ps.GetHeight()}");
		//
		//
		// 	if (pageRotation == 90) return null;
		//
		// 	if ( (pageRotation == 0 && ps.GetHeight() > ps.GetWidth()) ||
		// 		(pageRotation == 270 && ps.GetWidth() > ps.GetHeight())
		// 		) return false;
		//
		//
