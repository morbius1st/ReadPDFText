...
namespace SharedCode.ShDataSupport
{
	...
	/// </summary>
	public class TextAndLineSegmentData2
	{
		...
		public TextAndLineSegmentData2( string text,
			LineSegment bott,
			LineSegment top,int onOnPage = -1)
		{
			Text = text;
		...
		public double[] GetRectPath()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				// vertices[i * 2] = CornerPointsAdjusted[i].x;
				// vertices[i * 2 + 1] = CornerPointsAdjusted[i].y;
			}
