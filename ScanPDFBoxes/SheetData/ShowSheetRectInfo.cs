...
namespace ScanPDFBoxes.SheetData
{
	public class ShowSheetRectInfo
	{
		...
		public static void showStatus(ShowWhere where)
		{
			...
			string count = SheetDataManager2.SheetsCount <= 0 ? "no sheets" : SheetDataManager2.SheetsCount.ToString();
			...
			string s1 = SheetDataManager2.Path?.RootFolderPath ?? "** null **";
			string s2 = SheetDataManager2.Path?.FileName ?? "** null **";
	...
	#region show info for "program" - orig
		...
		private static string formatSingleRect2(string name, float rotn, SheetRectType t, Rectangle r, Rectangle pageSize = null)
		{
			...
			return $"{name,TITLE_WIDTH}| {type,TYPE_WIDTH}| {FormatItextData.FormatRectangle(r)} {rotation, 10}  {oa}";
		}
