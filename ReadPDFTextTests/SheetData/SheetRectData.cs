...
namespace ReadPDFTextTests.SheetData
{
	...
	public class SheetRectData<T, U>
		where U : class
	{
		...
		public SheetRectData(SheetRectType type, T id, U rect)
		{
			Type = type;
			Id = id;
			Rect = rect;
		...
		public SheetRectData(string name, T id, U rect)
		{
			Id = id;
			Rect = rect;

			Type = SheetMetricsSupport.GetShtRectType(name);

			if (Type == SheetRectType.SRT_NA)
			{
				Type = SheetMetricsSupport.GetOptRectType(name);
			}
